...
function oplus_configure_tuning_swappiness() {
	...
	# add by zhangpanpan for chongqing 4G memory-23604 tuning
	if [ $MemTotal -le 4194304 ]; then
...
function configure_para_chongqing()
{
	product_id=`getprop ro.boot.prjname`
	MemTotalStr=`cat /proc/meminfo | grep MemTotal`
	MemTotal=${MemTotalStr:16:8}
	...
	case "$product_id" in
		"23660"|"23661"|"23604"|"23670"|"23671"|"23605"|"23606")
